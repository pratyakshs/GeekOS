...
void Sys_IO_Unblock(int param) {
    // wake up
    // Wake_Up(&IOWaitQueue);
    // Make_Runnable()
    struct Thread_Queue * ToWakeUp = 0;
    Enqueue_Thread(ToWakeUp, IOWaitQueue->head);
    Wake_Up(ToWakeUp);
    Remove_Thread(IOWaitQueue, IOWaitQueue->head);
...
static int Sys_ReadBlock(struct Interrupt_State *state) {
    ...
    int blocknum = state->edi;
    int waitFor = get_time(blocknum);
    int id = Start_Timer(Calc_Ticks_Per_MS(waitFor), &Sys_IO_Unblock);
    struct Kernel_Thread *current = CURRENT_THREAD;

    Enqueue_Thread(IOWaitQueue, current); 
    Schedule();
    return EUNSUPPORTED;
}
...
static int Sys_OpenCount(struct Interrupt_State *state) {
	Print("Process made %i Open File Sys Calls\n",CURRENT_THREAD->open_count);
	return EUNSUPPORTED;
}

static int Sys_TotalCount(struct Interrupt_State *state) {
	Print("Process made %i Sys Calls\n",CURRENT_THREAD->total_count);
	return EUNSUPPORTED;
}

static int Sys_Get_NewTOD(struct Interrupt_State *state) {
    Copy_To_User(state->ebx, (void*)&g_numTicks, sizeof(int));
    return 0;
}
